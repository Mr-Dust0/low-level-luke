...
- **I said "process per client".** It's more accurately "process per connection". A single client can open multiple connections, and with external connection poolers like PgBouncer sitting in front of the database, many clients might share fewer backend processes. The distinction matters because connection pooling (which I mentioned at the end) exists precisely to decouple the number of clients from the number of processes.

- **I said lightweight threads don't give the kernel a chance to optimise thread priority, leading to performance issues.** This is only part of the picture. The bigger limitation of user-space threads is that threads scheduled on a single OS thread can't utilise multiple CPU cores. That's why real implementations use **M:N scheduling** - multiplexing many lightweight threads across a smaller number of OS threads (Go does exactly this with its goroutine scheduler, mapping goroutines onto `GOMAXPROCS` OS threads). The scheduling concern is real but secondary to the multi-core utilisation issue.
